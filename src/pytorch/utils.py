    ...
    def _read_bytes_to_tensor(self, path):
        with open(path, "rb") as bf:
            return torch.from_numpy(
                np.transpose(
                    np.reshape(np.frombuffer(bf.read(), dtype="float32"), self.shape),
                    (2, 0, 1),
                ).copy()
            )
...
def create_flow_view(model, batches, path, device):
    ...
    resize = torchvision.transforms.Resize(
        (batches[0][1].shape[2], batches[0][1].shape[3]), antialias=True
    )
...
def create_attention_view(model, batches, path, device):
    ...
    for child in model.named_children():
        if "attention" in child[0]:
            hooks.append(
                child[1].ocnn.register_forward_hook(
                    get_activation(child[0], child[1].out_act, child[1].upsample)
                )
            )
            names.append(child[0])
    ...
    gray2rgb = torchvision.transforms.Lambda(
...
def fit(
    ...
    for epoch in range(epochs):
        ...
        for step, record in enumerate(train):
            ...
            # extract the data
            left, right, y = (
                record[0][0].to(device),
                record[0][1].to(device),
                record[1].to(device),
            )
            ...
            if save_freq is not None and step % save_freq == 0 and step > 0:
                ...
                if mode == "all" or (
                    mode == "best" and (best_loss is None or best_loss > loss_avg)
                ):
                    filename = os.path.join(
                        target_path,
                        f"{name}_l={loss_avg}_e={epoch + 1}_s={step + 1}.pt",
                    )
            ...
            if log_perf_freq is not None and step % log_perf_freq == 0 and step > 0:
                ...
                create_output_view(
                    model,
                    vis_batches,
                    os.path.join(target_path, f"output_e={epoch + 1}_s={step + 1}.png"),
                    device,
                )
                create_flow_view(
                    model,
                    vis_batches,
                    os.path.join(target_path, f"flow_e={epoch + 1}_s={step + 1}.png"),
                    device,
                )
                create_attention_view(
                    model,
                    vis_batches,
                    os.path.join(
                        target_path, f"attention_e={epoch + 1}_s={step + 1}.png"
                    ),
                    device,
                )
                model.train(True)
        ...
        for step, record in enumerate(valid):
            ...
            for metric, value in zip(
                metrics, [metric(y, y_pred) for metric in metrics]
            ):
                loss_metrics[metric.__name__].append(value.item())
...
def save_history_plot(
    path, history, figsize=(10, 5), exclude=None, use_norm=True, steps=None
):
    def compress_loss(loss, steps):
        return [
            np.average(loss[steps * i : steps * (i + 1)])
            for i in range(len(loss) // steps)
        ]
    ...
    for index, (metric, values) in enumerate(history.items()):
        ...
        plt.plot(
            steps, history[f"val_{metric}"], colors[index] + "--", label=f"val_{metric}"
        )
