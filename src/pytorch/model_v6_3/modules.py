...
"""
This module contains fbnet v6_3 with the following features:
...
- Encoder with filters outputs on each level but included feature stacking on the input of the conv2dblock (each blocks input is a stack of every previous layers outputs properly resized by avgpooling d)
...
- Use of custom Conv2dBlock (in_channels -> in_channels -> out_channels, more conv2d layers at each level, 2 vs 1)
"""
...
class FlowFeatureWarp(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        assert len(fcounts) == len(fsizes) == len(fpads) == 5, (
            "Filter options should have the same size of 5 elements"
        )
        ...
        self.cnn_2 = nn.Conv2d(
            input_channels + fcounts[0], fcounts[1], fsizes[1], padding=fpads[1]
        )
        self.cnn_3 = nn.Conv2d(
            input_channels + fcounts[0] + fcounts[1],
            fcounts[2],
            fsizes[2],
            padding=fpads[2],
        )
        self.cnn_4 = nn.Conv2d(
            input_channels + fcounts[0] + fcounts[1] + fcounts[2],
            fcounts[3],
            fsizes[3],
            padding=fpads[3],
        )
        self.cnn_5 = nn.Conv2d(
            input_channels + fcounts[0] + fcounts[1] + fcounts[2] + fcounts[3],
            fcounts[4],
            fsizes[4],
            padding=fpads[4],
        )
    ...
    def forward(self, input_1, input_2, upsampled_flow):
        ...
        flow_upsampled = (
            self.upsample(flow) * 2
        )  # we multiply by 2 because of the upsampling the scale
...
class Conv2dBlock(nn.Module):
    def __init__(
        ...
        self.cnn_1 = nn.Conv2d(in_channels, in_channels, kernel_size, stride, padding)
        self.cnn_2 = nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding)
        self.act = nn.PReLU()
...
class FBNet(nn.Module):
    def __init__(self, input_shape, device, filters=[32, 32, 32, 32], **kwargs):
        ...
        self.cnn_block_r3 = Conv2dBlock(
            self.c + filters[0] + filters[1], filters[2], 3, 1, 1
        )
        self.cnn_block_r4 = Conv2dBlock(
            self.c + filters[0] + filters[1] + filters[2], filters[3], 3, 1, 1
        )

        self.downsample_1_2 = torchvision.transforms.Resize(
            (self.h // 2, self.w // 2), antialias=True
        )
        self.downsample_1_3 = torchvision.transforms.Resize(
            (self.h // 4, self.w // 4), antialias=True
        )
        self.downsample_1_4 = torchvision.transforms.Resize(
            (self.h // 8, self.w // 8), antialias=True
        )
        ...
        self.flow_warp_r1 = FlowFeatureWarp(input_channels=filters[0], level=1)
        self.flow_warp_r2 = FlowFeatureWarp(input_channels=filters[1], level=2)
        self.flow_warp_r3 = FlowFeatureWarp(input_channels=filters[2], level=3)
        self.flow_warp_r4 = FlowFeatureWarp(input_channels=filters[3], level=4)

        # ------------- Attention layers
        self.attention_r1 = AttentionGate(
            g_filters=filters[1], x_filters=filters[0], filters=filters[0]
        )
        self.attention_r2 = AttentionGate(
            g_filters=filters[2], x_filters=filters[1], filters=filters[1]
        )
        self.attention_r3 = AttentionGate(
            g_filters=filters[3], x_filters=filters[2], filters=filters[2]
        )

        # ------------- Feature decoding layers
        self.up_r4 = nn.Upsample(
            scale_factor=(2, 2), mode="bilinear", align_corners=True
        )

        self.conv_dec_block_r3 = Conv2dBlock(
            filters[2] + filters[3], filters[2], 3, 1, 1
        )
        self.up_r3 = nn.Upsample(
            scale_factor=(2, 2), mode="bilinear", align_corners=True
        )

        self.conv_dec_block_r2 = Conv2dBlock(
            filters[1] + filters[2], filters[1], 3, 1, 1
        )
        self.up_r2 = nn.Upsample(
            scale_factor=(2, 2), mode="bilinear", align_corners=True
        )

        self.conv_dec_block_r1 = Conv2dBlock(
            filters[0] + filters[1], filters[0], 3, 1, 1
        )
    ...
    def forward(self, left, right):
        ...
        # second conv block
        input_left_cnn_r2 = self.cnn_block_r2(
            torch.cat([left_2, intut_left_avg_r1_1], dim=1)
        )
        intut_left_avg_r2_1 = self.avg_r2_1(input_left_cnn_r2)
        intut_left_avg_r2_2 = self.avg_r2_2(input_left_cnn_r2)

        # third conv block
        input_left_cnn_r3 = self.cnn_block_r3(
            torch.cat([left_3, intut_left_avg_r1_2, intut_left_avg_r2_1], dim=1)
        )
        intut_left_avg_r3_1 = self.avg_r3_1(input_left_cnn_r3)

        # fourth conv block
        input_left_cnn_r4 = self.cnn_block_r4(
            torch.cat(
                [left_4, intut_left_avg_r1_3, intut_left_avg_r2_2, intut_left_avg_r3_1],
                dim=1,
            )
        )
        ...
        # second conv block
        input_right_cnn_r2 = self.cnn_block_r2(
            torch.cat([right_2, intut_right_avg_r1_1], dim=1)
        )
        intut_right_avg_r2_1 = self.avg_r2_1(input_right_cnn_r2)
        intut_right_avg_r2_2 = self.avg_r2_2(input_right_cnn_r2)

        # third conv block
        input_right_cnn_r3 = self.cnn_block_r3(
            torch.cat([right_3, intut_right_avg_r1_2, intut_right_avg_r2_1], dim=1)
        )
        intut_right_avg_r3_1 = self.avg_r3_1(input_right_cnn_r3)

        # fourth conv block
        input_right_cnn_r4 = self.cnn_block_r4(
            torch.cat(
                [
                    right_4,
                    intut_right_avg_r1_3,
                    intut_right_avg_r2_2,
                    intut_right_avg_r3_1,
                ],
                dim=1,
            )
        )
        ...
        # ------------- Warp features
        warp_r4, flow_r4 = self.flow_warp_r4(
            input_left_cnn_r4, input_right_cnn_r4, None
        )
        warp_r3, flow_r3 = self.flow_warp_r3(
            input_left_cnn_r3, input_right_cnn_r3, flow_r4
        )
        warp_r2, flow_r2 = self.flow_warp_r2(
            input_left_cnn_r2, input_right_cnn_r2, flow_r3
        )
