...
"""
This module contains fbnet v7_1
"""
...
class FlowPyramid(nn.Module):
    def __init__(self, in_channels, filters, fsizes, **kwargs):
        ...
        self.cnn_5 = nn.Conv2d(
            in_channels + filters, in_channels + 2 * filters, 3, 1, 1
        )
        self.cnn_6 = nn.Conv2d(
            in_channels + 2 * filters, in_channels + 2 * filters, 3, 2, 1
        )
        self.cnn_7 = nn.Conv2d(
            in_channels + 2 * filters, in_channels + 3 * filters, 3, 1, 1
        )

        # ------------- Flow estimation layers
        self.flow_1 = FlowEstimator(
            filters=in_channels, fcount=in_channels, fsize=fsizes[0]
        )
        self.flow_2 = FlowEstimator(
            filters=in_channels + filters, fcount=in_channels + filters, fsize=fsizes[1]
        )
        self.flow_3 = FlowEstimator(
            filters=in_channels + 2 * filters,
            fcount=in_channels + 2 * filters,
            fsize=fsizes[2],
        )
        self.flow_4 = FlowEstimator(
            filters=in_channels + 3 * filters,
            fcount=in_channels + 3 * filters,
            fsize=fsizes[3],
        )
...
class FBNet(nn.Module):
    def __init__(self, input_shape, device, filters=32, fsizes=[3, 3, 3, 3], **kwargs):
        ...
        self.cnn_decblock_1 = Conv2dBlock(
            2 * (self.c + 4 * filters), 4 * filters, 3, 1, 1
        )
        ...
        # ------------- Flow pyramid layer
        self.flow_pyramid = FlowPyramid(
            in_channels=self.c + 4 * filters, filters=0, fsizes=fsizes
        )
