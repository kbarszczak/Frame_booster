...
"""
This module contains fbnet v6_1 with the following features:
...
class FlowFeatureWarp(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            len(fcounts) == len(fsizes) == len(fstrides) == len(fpads) == len(fdils)
        ), "Filter options should have the same size"

        modules = []
        for fcount, fsize, fstride, fpad, fdil in zip(
            fcounts, fsizes, fstrides, fpads, fdils
        ):
            modules.append(
                nn.Conv2d(flow_input_chanels, fcount, fsize, fstride, fpad, fdil)
            )
...
class AttentionGate(nn.Module):
    def __init__(
        self, g_filters, x_filters, filters, interpolation="bilinear", **kwargs
    ):
