...
class FlowFeatureWarp(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        modules = []
        for fcount, fsize, fstride, fpad, fdil, fact in zip(
            fcounts, fsizes, fstrides, fpads, fdils, facts
        ):
            modules.append(
    ...
    def forward(self, input_1, input_2, upsampled_flow):
        ...
        estimated_flow = self.flow(
            torch.cat([input_1, input_2_warped_1, upsampled_flow], dim=1)
        )
...
class FBNet(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.zero_flow = torch.zeros(
            (self.b, 2, self.h // 8, self.w // 8), device=device
        )
    ...
    def forward(self, left, right):
        ...
        warp_r4, flow_r4 = self.flow_warp_r4(
            input_right_cnn_r4_1, input_left_cnn_r4_1, self.zero_flow
        )
