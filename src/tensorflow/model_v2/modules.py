...
class PyramidFeatureExtraction(layers.Layer):
    def __init__(
        ...
        **kwargs,
        ...
        self.cnn_1st_level = layers.Conv2D(
            filter_count[0],
            filter_size,
        ...
        self.cnn_2nd_level = layers.Conv2D(
            filter_count[1],
            filter_size,
            activation=activation,
            kernel_regularizer=regularizer,
        ...
        self.cnn_3rd_level = layers.Conv2D(
            filter_count[2],
            filter_size,
            activation=activation,
            kernel_regularizer=regularizer,
    ...
    def call(self, inputs):
        ...
        concat_2nd = self.concat_2nd_level(
            [input_2_column_1_row_2, input_1_column_2_row_2]
        )
        concat_3rd = self.concat_3rd_level(
            [input_3_column_1_row_3, input_2_column_2_row_3, input_1_column_3_row_3]
        )
        concat_4th = self.concat_4th_level(
            [input_4_column_1_row_4, input_3_column_2_row_4, input_2_column_3_row_4]
        )
...
class BidirectionalFlowEstimation(layers.Layer):
    def __init__(
        self,
        filter_count=[32, 64, 64, 16],
        filter_size=[(3, 3), (3, 3), (1, 1), (1, 1)],
        activation="relu",
        regularizer=None,
        ...
        **kwargs,
        ...
        self.flow_upsample_1_2 = layers.UpSampling2D(
        ...
        self.flow_prediction_1_2 = keras.Sequential(
            [
                ...
                layers.Conv2D(
                    2, (1, 1), kernel_regularizer=regularizer, padding="same"
                ),
            ]
        )
        ...
        self.flow_upsample_2_1 = layers.UpSampling2D(
            (2, 2), interpolation=interpolation
        )
...
class WarpedFeatureFusion(layers.Layer):
    def __init__(
        self,
        feature_extractor_filter_count=[16, 32, 64],
        filter_size=(3, 3),
        activation="relu",
        regularizer=None,
        interpolation="bilinear",
        **kwargs,
        ...
        self.cnn_1st_level_o = layers.Conv2D(
            3, (1, 1), activation=output_activation, padding="same"
        )
        self.add_1st_level = layers.Add()

        self.up_2nd_level = layers.UpSampling2D((2, 2), interpolation=interpolation)
        self.cnn_2nd_level_2 = layers.Conv2D(
            fefc[0],
            filter_size,
            activation=activation,
            kernel_regularizer=regularizer,
        ...
        self.cnn_2nd_level_1 = layers.Conv2D(
            fefc[0],
            filter_size,
            activation=activation,
            kernel_regularizer=regularizer,
        ...
        self.cnn_3rd_level_2 = layers.Conv2D(
            fefc[0] + fefc[1],
            filter_size,
            activation=activation,
            kernel_regularizer=regularizer,
        ...
        self.cnn_3rd_level_1 = layers.Conv2D(
            fefc[0] + fefc[1],
            filter_size,
            activation=activation,
            kernel_regularizer=regularizer,
        ...
        self.cnn_4th_level_2 = layers.Conv2D(
            fefc[0] + fefc[1] + fefc[2],
            filter_size,
            activation=activation,
            kernel_regularizer=regularizer,
            padding="same",
        )
        self.cnn_4th_level_1 = layers.Conv2D(
            fefc[0] + fefc[1] + fefc[2],
            filter_size,
            activation=activation,
            kernel_regularizer=regularizer,
...
class FBNet(layers.Layer):
    def __init__(
        ...
        **kwargs,
        ...
        assert len(pyramid_filter_count) == 3, (
            "pyramid_filter_count length is expected to be 3. Got: "
            + str(len(pyramid_filter_count))
        )
        assert len(pyramid_filter_size) == 2, (
            "pyramid_filter_size length is expected to be 2. Got: "
            + str(len(pyramid_filter_size))
        )
        assert len(flow_filter_count) == 4, (
            "flow_filter_count length is expected to be 4. Got "
            + str(len(flow_filter_count))
        )
        assert len(flow_filter_sizes) == 4, (
            "flow_filter_sizes length is expected to be 4. Got "
            + str(len(flow_filter_sizes))
        )

        # for building the pyramid
        self.pyramid_input = PyramidFeatureExtraction(
    ...
    def call(self, inputs, batch_size=1):
        ...
        feature_extraction_1 = self.pyramid_input(input_1)
        feature_extraction_2 = self.pyramid_input(input_2)

        # create empty flow for the coarest level
        empty_flow_1 = tf.zeros(
            shape=(
                batch_size,
                feature_extraction_1[3].shape[1],
                feature_extraction_1[3].shape[2],
                2,
            )
        )
        empty_flow_2 = tf.zeros(
            shape=(
                batch_size,
                feature_extraction_1[3].shape[1],
                feature_extraction_1[3].shape[2],
        ...
        bfe_4_i1, bfe_4_i2, bfe_4_f_1_2, bfe_4_f_2_1 = (
            self.bidirectional_flow_estimation_3(
                [
                    feature_extraction_1[3],
                    feature_extraction_2[3],
                    empty_flow_1,
                    empty_flow_2,
                ]
            )
        ...
        bfe_3_i1, bfe_3_i2, bfe_3_f_1_2, bfe_3_f_2_1 = (
            self.bidirectional_flow_estimation_3(
                [
                    feature_extraction_1[2],
                    feature_extraction_2[2],
                    bfe_4_f_1_2,
                    bfe_4_f_2_1,
                ]
            )
        ...
        bfe_2_i1, bfe_2_i2, bfe_2_f_1_2, bfe_2_f_2_1 = (
            self.bidirectional_flow_estimation_2(
                [
                    feature_extraction_1[1],
                    feature_extraction_2[1],
                    bfe_3_f_1_2,
                    bfe_3_f_2_1,
                ]
            )
        )
        bfe_1_i1, bfe_1_i2, _, _ = self.bidirectional_flow_estimation_1(
            [feature_extraction_1[0], feature_extraction_2[0], bfe_2_f_1_2, bfe_2_f_2_1]
        )

        # merge the features extracted by the previous layers
        outputs = self.warped_feature_fusion(
            [
