...
def get_parser():
    parser = argparse.ArgumentParser(
        description="Creating dataset for training FBNet model"
    )
    parser.add_argument(
        "-s",
        "--source",
        required=True,
        type=str,
        help="The source path of the raw dataset to process",
    )
    parser.add_argument(
        "-t",
        "--target",
        required=True,
        type=str,
        help="The target path where the results files will be created",
    )
    parser.add_argument(
        "-l",
        "--loader",
        ...
        type=str,
        default="vimeo90k",
        help="The name of the function that loads the data for tfrecords processor",
    )
    parser.add_argument(
        "-tr",
        "--train_limit",
        ...
        type=int,
        default=1000,
        help="The max samples count in the training set",
    )
    parser.add_argument(
        "-ts",
        "--test_limit",
        ...
        type=int,
        default=200,
        help="The max samples count in the testing set",
    )
    parser.add_argument(
        "-tv",
        "--valid_split",
        ...
        type=float,
        default=0.9,
        help="The train to validation split",
    )
    parser.add_argument(
        "-i",
        "--interpolation",
    ...
    parser.add_argument(
        ...
        type=int,
        default=256,
        help="The width of target images",
    )
    parser.add_argument(
        "-ih",
        "--height",
        ...
        type=int,
        default=144,
        help="The height of target images",
    )
    parser.add_argument(
        "-d",
        "--delay",
        ...
        type=int,
        default=5,
        help="The delay that is applied during loading frames from the custom dataset. It is used only with -l option set to 'custom'",
    )
...
def vimeo90k(
    ...
    train_paths, valid_paths = create_paths_vimeo90k(
        os.path.join(base_path, "tri_trainlist.txt"),
        limit=train_limit,
        split=train2valid_split,
    )
    test_paths = create_paths_vimeo90k(
        os.path.join(base_path, "tri_testlist.txt"), limit=test_limit, split=None
    )

    print("Processing train dataset ...")
    train_target_path = os.path.join(
        target_path, f"train_{size[1]}x{size[0]}_{len(train_paths)}.tfrecords"
    )
    create_tf_record_vimeo90k(
        os.path.join(base_path, "sequences"),
        train_paths,
        train_target_path,
        size,
        interpolation,
    ...
    test_target_path = os.path.join(
        target_path, f"test_{size[1]}x{size[0]}_{len(test_paths)}.tfrecords"
    )
    create_tf_record_vimeo90k(
        os.path.join(base_path, "sequences"),
        test_paths,
    ...
    valid_target_path = os.path.join(
        target_path, f"valid_{size[1]}x{size[0]}_{len(valid_paths)}.tfrecords"
    )
    create_tf_record_vimeo90k(
        os.path.join(base_path, "sequences"),
        valid_paths,
...
def custom(
    ...
    data = read_opposite_frames(
        base_path, size, interpolation, delay, train_limit + test_limit
    )
    ...
    create_tfrecord_custom(
        train,
        os.path.join(target_path, f"train_{size[1]}x{size[0]}_{len(train)}.tfrecords"),
    )
    create_tfrecord_custom(
        test,
        os.path.join(target_path, f"test_{size[1]}x{size[0]}_{len(test)}.tfrecords"),
    )
    create_tfrecord_custom(
        valid,
        os.path.join(target_path, f"valid_{size[1]}x{size[0]}_{len(valid)}.tfrecords"),
    )
...
if __name__ == "__main__":
    ...
    assert loader in ["vimeo90k", "custom"], (
        "The loader function can only be: vimeo90 (for vimeo90k triplet dataset) or custom (loads each video file from pointer directory)"
    )
    assert valid_split >= 0 or valid_split <= 1, (
        "Validation split is expected to be in range from 0 to 1"
    )
    assert interpolation in ["bicubic", "bilinear"], (
        "Interpolation is either bicubic or bilinear"
    )
