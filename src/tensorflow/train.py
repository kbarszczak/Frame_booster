...
def get_parser():
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="The source path of the training dataset",
    )
    parser.add_argument(
        "-trc",
        "--train_count",
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="The source path of the testing dataset",
    )
    parser.add_argument(
        "-tsc",
        "--test_count",
        required=True,
        type=int,
        help="The size of the testing dataset",
    )
    parser.add_argument(
        "-v",
        "--valid",
        required=True,
        type=str,
        help="The source path of the validating dataset",
    )
    parser.add_argument(
        "-vc",
        "--valid_count",
        required=True,
        type=int,
        help="The size of the validating dataset",
    )
    parser.add_argument(
        "-t",
        "--target_path",
        required=True,
        type=str,
        help="The path where models are saved during the training (models are saved after each 500 batches)",
    )
    parser.add_argument(
        "-n", "--name", required=True, type=str, help="The name of the created model)"
    )
    parser.add_argument(
        "-b", "--batch_size", required=False, type=int, default=5, help="The batch size"
    )
    parser.add_argument(
...
def evaluate(model, generator):
    ...
    for index, metric in enumerate(model.metrics):
        ...
        print(
            f"{metric.name.zfill(13).replace('0', ' ')}: {np.round(result[index], 4)}"
        )
...
def train(
    ...
    train_generator = load_generator(
        path=train_path, epochs=epochs, batch_size=batch_size
    )
    test_generator = load_generator(
        path=test_path, epochs=epochs, batch_size=batch_size
    )
    valid_generator = load_generator(
        path=valid_path, epochs=epochs, batch_size=batch_size
    )
    ...
    model_target_path = os.path.join(
        model_path,
        f"{model_name}_loss_{eval_dict['loss']}_l1_{eval_dict['l1']}_l2_{eval_dict['l2']}_psnr_{eval_dict['psnr']}_ssim_{eval_dict['ssim']}.h5",
    )
