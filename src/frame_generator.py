...
def get_parser():
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="The path of the source video file to process",
    )
    parser.add_argument(
        "-t",
        "--target",
        required=True,
        type=str,
        help="The path where the results videos will be created",
    )
    parser.add_argument(
        "-m",
        "--model",
        required=True,
        type=str,
        help="The path of model used during the video processing",
    )
    parser.add_argument(
        "-mv",
        "--model_version",
        ...
        type=str,
        default="v1",
        help="The version of the model (v1, v2, v3)",
    )
    parser.add_argument(
        "-vn",
        "--video_name",
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="The frame multiplying coefficient (2x, 4x, 8x, 16x)",
    )
    parser.add_argument(
        "-md",
        "--mode",
        ...
        type=str,
        default="low_mem",
        help="The processing mode. Either 'fast' or 'low_mem'",
    )
    parser.add_argument(
        "-lml",
        "--low_mem_limit",
        ...
        type=int,
        default=500,
        help="The limit of the input size for the net. Applies only if the mode is set to 'low_mem' (-md switch)",
    )
    parser.add_argument(
        "-iw",
        "--width",
...
def fast_boosting(
    source_path, target_path, cmp_target_path, model, count, size, fourcc
):
    ...
    original_frames = [
        cv2.cvtColor(frame, cv2.COLOR_BGR2RGB) for frame in original_frames
    ]
    ...
    out = cv2.VideoWriter(
        cmp_target_path, fourcc, (count * fps), (size[0] * 2, size[1])
    )
    for index in range(len(original_frames)):
        ...
        for index_generated in range(coef):
            if coef * index + index_generated >= len(frames):
                break
            frame = cv2.hconcat(
                [original_frames[index], frames[coef * index + index_generated]]
            )
            out.write(frame)
...
def low_mem_boosting(
    source_path, target_path, cmp_target_path, model, count, size, fourcc, mem_limit
):
    vidcap = cv2.VideoCapture(source_path)
    fps = vidcap.get(cv2.CAP_PROP_FPS)
    result_out = cv2.VideoWriter(target_path, fourcc, count * fps, size)
    cmp_result_out = cv2.VideoWriter(
        cmp_target_path, fourcc, (count * fps), (size[0] * 2, size[1])
    )
        ...
        if len(frames) >= 2:
            for _ in range(int(math.log2(count))):
                ...
                for part in range(int(math.ceil(len(frames) / mem_limit))):
                    ...
                    generated_frames = model(
                        [net_input[:, 0, :, :, :], net_input[:, 1, :, :, :]]
                    )
...
if __name__ == "__main__":
    ...
    assert extension in ["mp4", "avi"], (
        "Extension is expected to be one of the following: 'mp4', 'avi'"
    )
    assert model_version in ["v1", "v2", "v3"], (
        "The model version has to be one of [v1, v2, v3]"
    )
